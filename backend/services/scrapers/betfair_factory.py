...
class BetfairMode(Enum):
    ...
    API = "api"  # Official API (paid but reliable)
...
class BetfairFactory:
    ...
    def create(mode: str = None) -> BookmakerScraper:
        ...
        try:
            ...
            raise ValueError(
                f"Invalid BETFAIR_MODE: {mode}. Must be 'scraper' or 'api'"
            )

        if betfair_mode == BetfairMode.API:
            logger.info("Using Betfair API (official)")
